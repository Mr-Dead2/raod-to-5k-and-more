...
export async function loadReminder(){return (await idbGet(KEY))||DEFAULT;}
export async function saveReminder(patch){const cur=(await idbGet(KEY))||DEFAULT;const next={...cur,...patch};await idbSet(KEY,next);return next;}
...
export async function showReminderNow(body,title="Ready to run?"){if(permission()!=="granted")return;const options={body,icon:"icons/icon-192.png",badge:"icons/icon-192.png",tag:"run5k-reminder",vibrate:[80,40,80],data:{url:"./"}};try{const reg=await navigator.serviceWorker.ready;await reg.showNotification(title,options);}catch{try{new Notification(title,options);}catch{}}}
