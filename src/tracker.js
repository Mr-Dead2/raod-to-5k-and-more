...
export function useRunTracker(opts = {}) {
  ...
  // Keeps the ongoing "run in progress" notification in step with the run.
  // updateLiveRun throttles itself, so calling it on every tick is cheap;
  // `force` pushes an immediate rewrite at splits, pauses and resumes.
  const pushLive=(force=false,paused=false)=>{const sec=liveElapsed()/1000,km=distRef.current/1000;updateLiveRun({km,elapsedSec:sec,paceSec:km>0.05?sec/km:0,paused,force}).catch?.(()=>{})};
  const pushLiveRef=useRef(pushLive); pushLiveRef.current=pushLive;
  ...
  const onFix=useCallback(f=>{setAccuracy(f.accuracy);if(statusRef.current!=="tracking")return;if(f.accuracy!=null&&f.accuracy>MAX_ACCURACY_M)return;const p=smooth(f);if(!last.current){const iv0=optsRef.current.interval,ph0=iv0&&iv0.runSec>0&&iv0.walkSec>0?((liveElapsed()/1000)%(iv0.runSec+iv0.walkSec)<iv0.runSec?"run":"walk"):null;last.current=p;lastMoveAt.current=p.t;setPoints(pts=>[...pts,ph0?{...p,phase:ph0}:p]);return}const d=haversine(last.current,p),dt=Math.max((p.t-last.current.t)/1000,.001);if(d<MIN_STEP_M||d/dt>MAX_SPEED_MS)return;if(autoPausedRef.current){startedAt.current=Date.now();setAuto(false)}lastMoveAt.current=p.t;distRef.current+=d;setDistanceM(distRef.current);const segSpeed=d/dt;if(segSpeed>maxSpeedRef.current){maxSpeedRef.current=segSpeed;setMaxSpeedMs(segSpeed)}if(f.alt!=null&&isFinite(f.alt)){if(!alt.current)alt.current={smooth:f.alt,ref:f.alt};else{const a=alt.current;a.smooth+=.3*(f.alt-a.smooth);if(a.smooth-a.ref>=CLIMB_HYST_M){elevRef.current+=a.smooth-a.ref;a.ref=a.smooth;setElevGainM(elevRef.current)}else if(a.smooth<a.ref)a.ref=a.smooth}}
    const iv=optsRef.current.interval;let pointPhase=null;if(iv&&iv.runSec>0&&iv.walkSec>0){const pos=(liveElapsed()/1000)%(iv.runSec+iv.walkSec);pointPhase=pos<iv.runSec?"run":"walk";phaseDistRef.current={...phaseDistRef.current,[pointPhase]:phaseDistRef.current[pointPhase]+d};setPhaseDist(phaseDistRef.current);if(lastPhaseRef.current!==null&&pointPhase!==lastPhaseRef.current)optsRef.current.onPhaseChange?.(pointPhase);lastPhaseRef.current=pointPhase}last.current=p;setPoints(pts=>[...pts,pointPhase?{...p,phase:pointPhase}:p]);while(distRef.current/1000>=nextKm.current){const sec=liveElapsed()/1000,split=sec-splitBase.current,k=nextKm.current;splitBase.current=sec;nextKm.current+=1;setSplits(s=>[...s,split]);notifyRunKm(k,split).catch?.(()=>{});pushLiveRef.current(true)}setElapsedMs(liveElapsed())
  },[]);
  ...
  const startTicker=useCallback(()=>{clearInterval(ticker.current);ticker.current=setInterval(()=>{if(optsRef.current.autoPause&&statusRef.current==="tracking"&&!autoPausedRef.current&&last.current&&Date.now()-lastMoveAt.current>STOP_MS){baseMs.current=liveElapsed();setAuto(true);pushLiveRef.current(true,true)}setElapsedMs(liveElapsed());pushLiveRef.current()},250)},[]);
